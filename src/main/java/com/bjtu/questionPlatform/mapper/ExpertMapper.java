...
public interface ExpertMapper {
    ...
    @Select("select * from keyword where keyscontent = #{keyscontent}")
    List<String> searchKeysidBykeyscontent(String keyscontent);
    ...
    @Insert("insert into expert (expertName,keysid,expertType,mail,phone,expertUnit,expertInformation) "+
            "values (#{expertName},#{keysId},1,#{mail},#{phone},#{expertUnit},#{expertInformation})")
        //插入的专家类型暂时没想好咋写

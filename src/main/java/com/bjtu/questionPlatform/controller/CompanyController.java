...
//        System.out.println(expertReport.getReportId());
    ...
    @CrossOrigin
    @ResponseResultBody
    @PostMapping(value = "/set_getKeyWords")
    public HashMap<String, Object> set_getKeyWords() {
        HashMap<String, Object> data = new HashMap<>();
        List<KeyWord> keyWordList=expertService.getAllKeys();
        List<KeyWord> arrays = keyWordList.stream().collect(Collectors.collectingAndThen(Collectors.toCollection(()
                -> new TreeSet<>(Comparator.comparing(n->n.getKeysContent()))), ArrayList::new));
        System.out.println(arrays);
        data.put("keys",arrays);
...
//        System.out.println(data);
    ...
    public void register(@RequestBody Expert expert) {
        expert.setKeysId(expertService.searchKeysidBykeyscontent(expert.getKeysId()).get(0));
